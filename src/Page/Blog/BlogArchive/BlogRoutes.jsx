...
const {id} = useParams()
const BlogPost = product.find(post => post.id === parseInt(id))
   

const [searchItem, setSearchItem] = useState("");
const [selectpost, setselectpost] = useState(BlogPost || product[0])
...
<div className='pt-10'>
     <div className='bg-[#FFF6F7] text-black p-15'>
          <h1 className='text-2xl md:text-4xl uppercase font-bold'>Blog Post</h1>
...
<div className='flex flex-col gap-3'>
     <p><Link  to={`/blogsingle/${BlogPost.id}`} className=' rounded-lg bg-orange-500 p-2 text-white font-bold w-20 text-center cursor-pointer'>{selectpost.category}</Link></p>
     <p><Link  to={`/blogsingle/${BlogPost.id}`}className='md:text-2xl font-bold  capitalize hover:text-red-400'>{selectpost.name}</Link></p>
